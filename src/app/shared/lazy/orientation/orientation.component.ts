...
export class OrientationComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: Orientation;

  // Store Data
  public dataList: Orientation[] = [];
  public filteredDataList: ReplaySubject<Orientation[]> = new ReplaySubject<Orientation[]>(1);
  searchDataList: Orientation[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allOrientations = [...this.searchDataList, ...this.dataList]
              const selectedData = allOrientations.filter(m => {
          ...
          return this.orientationService.getAllOrientation(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  /**
   * HTTP REQ HANDLE
   * getAllOrientationList()
   */

  private getAllOrientationList() {
    ...
    this.orientationService.getAllOrientation(filter, null)
      .subscribe({
        next: res => {
