...
export class TypeSelectComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  public filteredDataList: ReplaySubject<Type[]> = new ReplaySubject<Type[]>(1);
  searchDataList: Type[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allTypes = [...this.searchDataList, ...this.dataList]
              const selectedData = allTypes.filter(m => {
          ...
          return this.typeService.getAllType(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  private getAllTypeList() {
    ...
    this.typeService.getAllType(filter, null)
      .subscribe({
        next: res => {
