...
export class RegionComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: Region;

  // Store Data
  public dataList: Region[] = [];
  public filteredDataList: ReplaySubject<Region[]> = new ReplaySubject<Region[]>(1);
  searchDataList: Region[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allRegions = [...this.searchDataList, ...this.dataList]
              const selectedData = allRegions.filter(m => {
          ...
          return this.regionService.getAllRegion(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  private getAllRegionList() {
    ...
    this.regionService.getAllRegion(filter, null)
      .subscribe({
        next: res => {
