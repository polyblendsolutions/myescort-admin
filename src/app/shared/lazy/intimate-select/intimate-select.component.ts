...
@Component({
  selector: 'app-intimate-select',
  templateUrl: './intimate-select.component.html',
  styleUrls: ['./intimate-select.component.scss']
})
export class IntimateSelectComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: IntimateHair;

  // Store Data
  public dataList: IntimateHair[] = [];
  public filteredDataList: ReplaySubject<IntimateHair[]> = new ReplaySubject<IntimateHair[]>(1);
  searchDataList: IntimateHair[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allIntimateHairs = [...this.searchDataList, ...this.dataList]
              const selectedData = allIntimateHairs.filter(m => {
          ...
          return this.intimateHairService.getAllIntimateHair(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  /**
   * HTTP REQ HANDLE
   * getAllIntimateHairList()
   */

  private getAllIntimateHairList() {
    ...
    this.intimateHairService.getAllIntimateHair(filter, null)
      .subscribe({
        next: res => {
